...
// D2D Conv Manager
typedef void(* conv_discovery_cb)(conv_device_h device_handle,
		conv_discovery_result_e result, void* user_data);
int conv_create(conv_h* handle);
int conv_destroy(conv_h handle);
int conv_discovery_start(conv_h handle, const int timeout_seconds,
...
int conv_payload_get_string(conv_payload_h handle,
		const char* key, char** value);
//int conv_payload_set_app_control(conv_payload_h handle,
//		const char* key, app_control_h app_control);
//int conv_payload_get_app_control(conv_payload_h handle,
//		const char* key, app_control_h* app_control);
...
// Service
typedef void(* conv_service_connected_cb)(conv_service_h service_handle,
...
// Device
typedef void(* conv_service_foreach_cb)(conv_service_h service_handle,
		void* user_data);
